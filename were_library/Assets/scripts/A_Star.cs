...
public class A_Star {
	...
	private List<a_star_node> a_star(a_star_node start, a_star_node goal)
	{
		...
		while (openset.Count > 0) {
			...
			foreach(a_star_node neighbor in neighbors(current)){
				...
				float tenative_score = get_score(g_score, current) + get_f_score(neighbor);
	...
	private a_star_node lowest_f_score(List<a_star_node> set) //SHOULD INCLUDE F (THIS IS JUST G SCORE
	{
		a_star_node to_return = set [0];
		
		
		foreach(a_star_node n in set){
			if(get_f_score(n) < get_f_score(to_return)){
				to_return = n;
			}
		}
	...
	private float get_f_score(a_star_node n){
